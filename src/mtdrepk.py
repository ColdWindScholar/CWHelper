...
def repack(mtd, type, erase, size):
    ...
    if type == "squashfs":
        ...
        call(['mksquashfs',
              mtd + "_unpacked",
              mtd + "_new",
              '-comp', comp, '-noappend', '-b', block, '-no-xattrs', '-always-use-fragments', '-all-root', '-Xbcj', filter_, '-Xdict-size', dict_size])
        print("已打包squashfs分区：",mtd)
    elif type == "jffs2":
        _, output = call(['mkfs.jffs2', '-d', mtd + "_unpacked", '-o', mtd + "_new", '-X', 'lzo', '--pagesize=0x1000', f'--eraseblock={erase}', '-l', '-n', '-q', '-v'], out=1, return_output=True)
